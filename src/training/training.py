...
# This notebook uses TensorFlow native tools for:
...
print(f"\n🎯 This layer will be part of the model - no separate preprocessing needed!")
...
print(f"\n🚀 These datasets will efficiently feed data to GPU during training!")
...
print("\n🚀 Fully TensorFlow-native training pipeline!")
...
print("\n1️⃣ Testing model on obvious examples:")
...
for product_type, text in obvious_examples.items():
    ...
    print(f"{product_type:20s} → Model predicts class {pred_class:2d} (confidence: {confidence:.1%})")

    if confidence > 0.5:  # Only trust high confidence predictions
        model_learned_mapping[pred_class] = product_type

print(f"\n2️⃣ Model's learned mapping (high confidence only):")
for class_id in sorted(model_learned_mapping.keys()):
    print(f"   Class {class_id:2d} → {model_learned_mapping[class_id]}")

# Now check against our actual products
print(f"\n3️⃣ Comparing with actual product names in data:")
...
print(f"\n4️⃣ All products in dataset (alphabetical):")
...
# ===================================================================
# Reconstruct Original 18-Class Mapping 
# ===================================================================
...
reconstructed_mapping = {
    ...
    16: "Prepaid card",
    17: "Virtual currency",  # CONFIRMED: bank account → class 17
}

print("\n1️⃣ Reconstructed mapping (from model behavior):")
...
print("\n2️⃣ Re-encoding all labels with reconstructed mapping...")
...
print("\n3️⃣ Evaluating with reconstructed mapping...")
...
if results[1] > 0.5:
    ...
    print("\n4️⃣ Manual verification with test cases:")
    ...
    for text, expected_class, product_name in test_cases:
        ...
        match = "✅" if pred_class == expected_class else "❌"
        print(f"{match} Text: {text[:40]:40s} → Predicted: {pred_class:2d}, Expected: {expected_class:2d}")
...
print("\n1️⃣ Testing ALL products systematically:\n")
...
for product, test_text in product_test_cases.items():
    ...
    print(f"{status} Class {pred_class:2d} ({confidence:5.1%}) ← {product[:55]}")
...
print(f"\n2️⃣ High-confidence mappings ({len(improved_mapping)} classes):")
final_mapping = {}
...
if unmapped_products:
    print(f"\n⚠️  {len(unmapped_products)} products not confidently mapped:")
    for product in unmapped_products:
        pred_class, conf = confidence_scores[product]
        print(f"   {product[:60]:60s} → Class {pred_class} ({conf:.1%})")
...
print("\n5️⃣ Evaluating with improved mapping...")
...
print("\n6️⃣ Generating classification report...")
...
"""## 6.3 Save Complete Model (with Preprocessing)
"""
