...
def main():
    ...
    logger.info(f"   Classes: {label_encoder.classes_[:5]}..." if num_classes > 5 else f"   Classes: {list(label_encoder.classes_)}")
    ...
    if args.use_class_weights:
        ...
        logger.info(f"✅ Class weights computed (range: {min(class_weights):.2f} - {max(class_weights):.2f})")
